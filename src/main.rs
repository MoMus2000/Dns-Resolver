...
use dns_parser::rdata::ns::Record;
use dns_parser::{self, Builder, Packet, Question, RData};
...
fn dns_query(question: &Question, root_servers: Vec<&str>){
    ...
    for server in root_servers{
        ...
        println!("Is Authoritative: {:?}", packet.header.authoritative);
        if !packet.header.authoritative{
            println!("Answers: {:?}", packet.answers);
            let nses = packet.nameservers;
            for ns in nses {
                match ns.data{
                    RData::NS(Record(ip)) => {
                        let name_server_address = ip.to_string();
                        let inner_sock = socket.try_clone().unwrap();
                        ...
                        let mut buf = [0; 512];
                        let _ = inner_sock.connect(name_server_address.to_string()+":53").unwrap();
                        let _ = socket.send(&bytes).unwrap();
                        let nbytes = socket.recv(&mut buf).unwrap();
                        let packet = Packet::parse(&buf[..nbytes]).unwrap();
